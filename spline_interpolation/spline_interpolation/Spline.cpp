...
Spline::Spline(const std::vector<double>& x, const std::vector<double>& y) : x(x) {
    ...
    {
        if(k != 0)
            b[k] = с[k] * h[k - 1] / 3 + с[k - 1] * h[k - 1] / 6 + ((y[k] - y[k - 1]) / h[k - 1]);
        else
            b[k] = с[k + 1] * h[k] / 3 + ((y[k + 1] - y[k]) / h[k]);
    }
...
double Spline::getSplineValue(size_t index, double x)
{
    ...
    double value = (a[index] + b[index] * epsilon + (с[index] / 2) * epsilon * epsilon +
